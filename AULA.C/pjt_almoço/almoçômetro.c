...
int main() {
    int almocos[7];           // vetor com 7 posições (um para cada dia da semana)
    char *dias[7] = {"Domingo", "Segunda", "Terça", "Quarta", "Quinta", "Sexta", "Sábado"};
    int i, total = 0;
    float media;
    ...
    printf("Dia de maior consumo: %s (%d almoços)\n", dias[diaPico], almocos[diaPico]);

    //  Menu interativo com while e switch-case 
    int opcao = 0;
    while (opcao != 8) {
        printf("\n=== Consulta ===\n");
        printf("Digite um número de 1 a 7 para ver os almoços de um dia (1=Domingo, 7=Sábado).\n");
